...
import type {// src/hooks/usePortfolios.ts
import { useMutation, useQuery, useQueryClient, keepPreviousData } from "@tanstack/react-query";
import { apiFetch, post, API_URL, authHeaders, del } from "@/lib/api";
import type {
...
export function usePublicPortfolios(page = 1, pageSize = 10) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<Paginated<PublicPortfolioRow>>(`/api/public-portfolios/?page=${page}&page_size=${pageSize}`),
...
export function usePublicTrades(id: number, page = 1, opts: { enabled?: boolean } = {}, pageSize = 10) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<Paginated<Trade>>(`/api/portfolios/${id}/trades/?page=${page}&page_size=${pageSize}`),
...
export function useDeletePortfolio(id: number) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.removeQueries({ queryKey: ["trades", id], exact: false });
      qc.removeQueries({ queryKey: ["allocations", id], exact: false });
      qc.removeQueries({ queryKey: ["chart", id], exact: false });
    },
  });
}
...
export function useCashIn(id: number) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["chart", id] });
    },
  });
}
